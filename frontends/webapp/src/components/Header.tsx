...
export default function Header({ onCreatePet, isCreateDisabled = false }: HeaderProps) {
  return (
    <header 
    ...
    >
      ...
      {onCreatePet && (
        <div className="flex-1 flex justify-center">
          <motion.button
            onClick={onCreatePet}
            className="relative rounded-lg bg-gradient-to-br from-purple-600/90 via-pink-600/90 to-rose-600/90 backdrop-blur-xl border border-white/20 overflow-hidden group h-6 sm:h-6 md:h-8 w-40 sm:w-44 md:w-52 max-w-[calc(100%-2rem)] disabled:opacity-60 disabled:cursor-not-allowed transition-transform duration-150"
            disabled={isCreateDisabled}
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: isCreateDisabled ? 0.7 : 1, y: 0, scale: isCreateDisabled ? 0.95 : 1 }}
            transition={{ delay: 0.1, duration: 0.3 }}
            whileHover={!isCreateDisabled ? { scale: 1.02, y: -1 } : {}}
            whileTap={!isCreateDisabled ? { scale: 0.97 } : {}}
          >
            ...
            <div className="relative flex items-center justify-center gap-1 sm:gap-1 px-1.5 sm:px-2.5 md:px-3 py-0.5 sm:py-0.5 md:py-1 h-full">
              {isCreateDisabled ? (
                <>
                  <span className="block h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 rounded-full border-2 border-white/70 border-t-transparent animate-spin" />
                  <span className="text-[0.6875rem] sm:text-xs md:text-sm font-semibold text-white truncate">Создание...</span>
                </>
              ) : (
                <>
