...
const queryClientSubscribers = new Set<{ queryClient: any; userId: string }>() // Подписчики queryClient для обработки сообщений
  ...
  try {
    ...
    queryClientSubscribers.forEach(({ queryClient, userId: subUserId }) => {
      ...
      switch (message.type) {
        ...
        case 'pet_created':
          queryClient.invalidateQueries({ queryKey: ['allPets', userId] })
...
function connectWebSocket(userId: string) {
  ...
  try {
    ...
    let apiBase = buildUrl.api('').replace(/\/api$/, '')
    
    // Если это относительный путь (dev режим), подключаемся напрямую к backend
    // Vite не может проксировать WebSocket, поэтому используем прямой URL
    if (!apiBase || apiBase.startsWith('/')) {
      // В dev режиме используем прямой backend URL (localhost:8080)
      apiBase = 'ws://localhost:8080'
    } else {
      // Для абсолютных URL просто меняем протокол
      apiBase = apiBase.replace(/^https/, 'wss').replace(/^http/, 'ws')
    }
    
    const wsUrl = `${apiBase}/api/ws/pets/${userId}`
