...
export class LinearMovementStrategy {
    ...
    rotateToTarget(target, delta) {
        ...
        // Интерпретируем rotationSpeed как скорость поворота в радианах в секунду.
        // Если значение не задано или некорректно, используем небольшой дефолт.
        const rotationSpeedRad = (typeof this.rotationSpeed === 'number' && this.rotationSpeed > 0)
            ? this.rotationSpeed
            : 0.15;
        
        // Вычисляем максимальный поворот за этот кадр (delta в миллисекундах)
        const maxRotation = rotationSpeedRad * (delta / 1000);
